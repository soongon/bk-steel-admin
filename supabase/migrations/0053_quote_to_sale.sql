-- ============================================================
-- 0053_quote_to_sale.sql
-- 견적 → 수주(매출) 전환. 원자적 RPC — sale 생성 + quote_line→sale_line 복사 + quote.status='won'.
--   · sale.source_quote_id 로 추적(견적 출처).
--   · 거래처: 견적에 있으면 그대로, 없으면(prospect만) overrides.partner_id 필수.
--   · 금액·자료성(is_documented·vat)은 견적 헤더 복사. tax_doc_type·납품일·수금예정일·doc_no 는 전환 시 지정(overrides).
--   · book 은 견적 그대로(BK 자료·B 무자료는 견적 작성 때 이미 결정).
-- ============================================================

ALTER TABLE sale ADD COLUMN IF NOT EXISTS source_quote_id UUID REFERENCES quote(id);
CREATE INDEX IF NOT EXISTS idx_sale_source_quote
  ON sale(source_quote_id) WHERE source_quote_id IS NOT NULL;
...
BEGIN
  ...
  -- 거래처: overrides 우선(잠재 고객 견적), 없으면 견적의 거래처. 둘 다 없으면 거부.
